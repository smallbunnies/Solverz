...
def made_numerical(eqs: SymEquations,
    ...
    from Solverz.equation.eqn import LoopEqnDiff as _LoopEqnDiff
    for eqn_name, eqn in eqs.EQNs.items():
        if isinstance(eqn, LoopEqn):
            custom_func[f'_sz_loop_{eqn_name}'] = eqn.NUM_EQN
        # Register Phase J3 LoopEqnDiff kernels under their unique
        # ``_sz_loop_jac_kernel_<sanitized>`` names so the J_ source
        # generated by the inline printer (via ``MutableMatJacData``
        # which prints the kernel-call Function application as a
        # bare name-call) can resolve them at runtime.
        for ed in eqn.derivatives.values():
            if isinstance(ed, _LoopEqnDiff):
                custom_func[ed._kernel_func_name] = ed.NUM_EQN
