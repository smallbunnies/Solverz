...
def test_numpy_code_printer():
    ...
    y = Var('y')
    k = idx('k')
    fxyk = lambdify([x, y, k], x[[1, 2, 3]] + y[k], modules=modules)
    assert (inspect.getsource(fxyk) ==
            "def _lambdifygenerated(x, y, k):\n    return x[[1, 2, 3]] + y[k]\n")
    x = np.array([1, 2, 3, 4, 5, 6]).reshape((-1, 1))
    y = np.array([-1, -2, -3, -7]).reshape((-1, 1))
    k = np.array([0, 3, 3])
    assert np.isclose(fxyk(x, y, k), np.array([[1], [-4], [-3]])).all()
    ...
    assert (inspect.getsource(fGk) ==
            "def _lambdifygenerated(k, G):\n    return G[k,sol_slice(None, None, None)]\n")
    ...
    assert (inspect.getsource(fkjG) ==
            "def _lambdifygenerated(k, j, G):\n    return G[sol_slice(None, None, None),sol_slice(1, k[j], 2)]\n")
    G = np.random.randn(3, 3)
    k = np.array([0, 2])
    j = [1]
